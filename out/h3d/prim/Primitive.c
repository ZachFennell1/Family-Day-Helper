// Generated by HLC 4.2.4 (HL v4)
#define HLC_BOOT
#include <hlc.h>
#include <h3d/prim/Primitive.h>
int h3d_Buffer_totalVertices(h3d__Buffer);
#include <h3d/col/Ray.h>
#include <h3d/col/Sphere.h>
#include <h3d/col/Frustum.h>
#include <h3d/Matrix.h>
#include <h3d/col/Point.h>
extern String s$not_implemented_for_;
String Std_string(vdynamic*);
String String___add__(String,String);
vdynamic* haxe_Exception_thrown(vdynamic*);
#include <h3d/col/Bounds.h>
#include <h3d/Engine.h>
extern String s$not_implemented;
#include <h3d/impl/ManagedBuffer.h>
#include <h3d/impl/MemoryManager.h>
void h3d_Engine_renderBuffer(h3d__Engine,h3d__Buffer,h3d__Indexes,int,int*,int*);
void h3d_Engine_renderIndexed(h3d__Engine,h3d__Buffer,h3d__Indexes,int*,int*);
void h3d_Buffer_dispose(h3d__Buffer);
void h3d_Indexes_dispose(h3d__Indexes);
String h3d_prim_Primitive_toString(h3d__prim__Primitive);
#include <hl/types/ArrayObj.h>
hl__Class Type_getClass(vdynamic*);
String Type_getClassName(hl__Class);
extern String s$5058f1a;
hl__types__ArrayObj String_split(String,String);
vdynamic* hl_types_ArrayObj_pop(hl__types__ArrayObj);

void h3d_prim_Primitive_new(h3d__prim__Primitive r0) {
	int r1;
	r1 = 0;
	r0->refCount = r1;
	return;
}

int h3d_prim_Primitive_triCount(h3d__prim__Primitive r0) {
	h3d__Buffer r5;
	double r2, r4;
	int r3;
	h3d__Indexes r1;
	r1 = r0->indexes;
	if( !r1 ) goto label$324c01e_2_10;
	r1 = r0->indexes;
	if( r1 == NULL ) hl_null_access();
	r3 = r1->count;
	r2 = (double)r3;
	r4 = 3.;
	r2 = r2 / r4;
	r3 = (int)r2;
	return r3;
	label$324c01e_2_10:
	r5 = r0->buffer;
	if( r5 ) goto label$324c01e_2_14;
	r3 = 0;
	return r3;
	label$324c01e_2_14:
	r5 = r0->buffer;
	if( r5 == NULL ) hl_null_access();
	r3 = h3d_Buffer_totalVertices(r5);
	r2 = (double)r3;
	r4 = 3.;
	r2 = r2 / r4;
	r3 = (int)r2;
	return r3;
}

int h3d_prim_Primitive_vertexCount(h3d__prim__Primitive r0) {
	int r1;
	r1 = 0;
	return r1;
}

vvirtual* h3d_prim_Primitive_getCollider(h3d__prim__Primitive r0) {
	String r2, r3;
	vdynamic *r1;
	r2 = (String)s$not_implemented_for_;
	r3 = Std_string(((vdynamic*)r0));
	r2 = String___add__(r2,r3);
	r1 = haxe_Exception_thrown(((vdynamic*)r2));
	hl_throw((vdynamic*)r1);
}

h3d__col__Bounds h3d_prim_Primitive_getBounds(h3d__prim__Primitive r0) {
	String r2, r3;
	vdynamic *r1;
	r2 = (String)s$not_implemented_for_;
	r3 = Std_string(((vdynamic*)r0));
	r2 = String___add__(r2,r3);
	r1 = haxe_Exception_thrown(((vdynamic*)r2));
	hl_throw((vdynamic*)r1);
}

void h3d_prim_Primitive_incref(h3d__prim__Primitive r0) {
	int r1;
	r1 = r0->refCount;
	++r1;
	r0->refCount = r1;
	return;
}

void h3d_prim_Primitive_decref(h3d__prim__Primitive r0) {
	int r1, r2;
	r1 = r0->refCount;
	--r1;
	r0->refCount = r1;
	r1 = r0->refCount;
	r2 = 0;
	if( r2 < r1 ) goto label$324c01e_7_9;
	r1 = 0;
	r0->refCount = r1;
	((void (*)(h3d__prim__Primitive))r0->$type->vobj_proto[7])(r0);
	label$324c01e_7_9:
	return;
}

void h3d_prim_Primitive_alloc(h3d__prim__Primitive r0,h3d__Engine r1) {
	String r3;
	vdynamic *r2;
	r3 = (String)s$not_implemented;
	r2 = haxe_Exception_thrown(((vdynamic*)r3));
	hl_throw((vdynamic*)r2);
}

void h3d_prim_Primitive_selectMaterial(h3d__prim__Primitive r0,int r1) {
	return;
}

h3d__prim__Primitive h3d_prim_Primitive_buildNormalsDisplay(h3d__prim__Primitive r0) {
	String r2, r3;
	vdynamic *r1;
	r2 = (String)s$not_implemented_for_;
	r3 = Std_string(((vdynamic*)r0));
	r2 = String___add__(r2,r3);
	r1 = haxe_Exception_thrown(((vdynamic*)r2));
	hl_throw((vdynamic*)r1);
}

void h3d_prim_Primitive_render(h3d__prim__Primitive r0,h3d__Engine r1) {
	vvirtual *r6;
	bool r4;
	h3d__impl__MemoryManager r10;
	h3d__Buffer r3;
	int *r11, *r13;
	int r8, r9, r12, r14, r15;
	h3d__Indexes r7;
	h3d__impl__ManagedBuffer r5;
	r3 = r0->buffer;
	if( !r3 ) goto label$324c01e_11_14;
	r3 = r0->buffer;
	if( r3 == NULL ) hl_null_access();
	r5 = r3->buffer;
	if( !r5 ) goto label$324c01e_11_12;
	r5 = r3->buffer;
	if( r5 == NULL ) hl_null_access();
	r6 = r5->vbuf;
	if( !r6 ) goto label$324c01e_11_12;
	r4 = false;
	goto label$324c01e_11_13;
	label$324c01e_11_12:
	r4 = true;
	label$324c01e_11_13:
	goto label$324c01e_11_15;
	label$324c01e_11_14:
	r4 = true;
	label$324c01e_11_15:
	if( !r4 ) goto label$324c01e_11_17;
	((void (*)(h3d__prim__Primitive,h3d__Engine))r0->$type->vobj_proto[4])(r0,r1);
	label$324c01e_11_17:
	r7 = r0->indexes;
	if( r7 ) goto label$324c01e_11_50;
	r3 = r0->buffer;
	if( r3 == NULL ) hl_null_access();
	r8 = r3->flags;
	r9 = 4;
	r8 = r8 & r9;
	r9 = 0;
	if( r8 == r9 ) goto label$324c01e_11_38;
	if( r1 == NULL ) hl_null_access();
	r3 = r0->buffer;
	r10 = r1->mem;
	if( r10 == NULL ) hl_null_access();
	r7 = r10->quadIndexes;
	r8 = 2;
	r9 = 0;
	r11 = &r9;
	r12 = -1;
	r13 = &r12;
	h3d_Engine_renderBuffer(r1,r3,r7,r8,r11,r13);
	goto label$324c01e_11_49;
	label$324c01e_11_38:
	if( r1 == NULL ) hl_null_access();
	r3 = r0->buffer;
	r10 = r1->mem;
	if( r10 == NULL ) hl_null_access();
	r7 = r10->triIndexes;
	r8 = 3;
	r14 = 0;
	r11 = &r14;
	r15 = -1;
	r13 = &r15;
	h3d_Engine_renderBuffer(r1,r3,r7,r8,r11,r13);
	label$324c01e_11_49:
	goto label$324c01e_11_56;
	label$324c01e_11_50:
	if( r1 == NULL ) hl_null_access();
	r3 = r0->buffer;
	r7 = r0->indexes;
	r11 = NULL;
	r13 = NULL;
	h3d_Engine_renderIndexed(r1,r3,r7,r11,r13);
	label$324c01e_11_56:
	return;
}

void h3d_prim_Primitive_dispose(h3d__prim__Primitive r0) {
	h3d__Buffer r2;
	h3d__Indexes r3;
	r2 = r0->buffer;
	if( !r2 ) goto label$324c01e_12_7;
	r2 = r0->buffer;
	if( r2 == NULL ) hl_null_access();
	h3d_Buffer_dispose(r2);
	r2 = NULL;
	r0->buffer = r2;
	label$324c01e_12_7:
	r3 = r0->indexes;
	if( !r3 ) goto label$324c01e_12_14;
	r3 = r0->indexes;
	if( r3 == NULL ) hl_null_access();
	h3d_Indexes_dispose(r3);
	r3 = NULL;
	r0->indexes = r3;
	label$324c01e_12_14:
	return;
}

vbyte* h3d_prim_Primitive___string(h3d__prim__Primitive r0) {
	String r2;
	vbyte *r1;
	r2 = h3d_prim_Primitive_toString(r0);
	if( r2 == NULL ) hl_null_access();
	r1 = r2->bytes;
	return r1;
}

String h3d_prim_Primitive_toString(h3d__prim__Primitive r0) {
	String r3, r5;
	hl__Class r4;
	hl__types__ArrayObj r2;
	vdynamic *r1;
	r4 = Type_getClass(((vdynamic*)r0));
	r3 = Type_getClassName(r4);
	if( r3 == NULL ) hl_null_access();
	r5 = (String)s$5058f1a;
	r2 = String_split(r3,r5);
	if( r2 == NULL ) hl_null_access();
	r1 = hl_types_ArrayObj_pop(r2);
	r3 = (String)r1;
	return r3;
}

